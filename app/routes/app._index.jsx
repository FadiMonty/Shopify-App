...
export const action = async ({ request }) => {
  ...
  const config = {
    ...
    merchantPass: formData.get("merchantPass"),
    test: await verifyTestMode(formData.get("test")),
    sessionId: session.id,
  };
  const ifExists = await getConfiguration(session.id);
  if (!ifExists) {
    ...
    const configuration = await updateConfiguration(config);
    const client = new PaymentsAppsClient(session.shop, session.accessToken);
    const response = await client.paymentsAppConfigure(
      configuration?.merchantKey,
    );
...
export default function Index() {
  ...
  const [merchantPass, setMerchantPass] = useState(
    config ? config.merchantPass : "",
  );
  const [testMode, setTestMode] = useState(config ? config.test : false);
